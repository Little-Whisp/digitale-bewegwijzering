...
export const getWeekForecastWeather = (response, descriptions_list) => {
  console.log('Week Forecast Response:', response);
...
export const getTodayForecastWeather = (
  ...
  console.log('Today Forecast Response:', response);
  let all_today_forecasts = [];
  

  if (!response || !response.list || response.list.length === 0 || response.cod === '404')
    return [];
  else
